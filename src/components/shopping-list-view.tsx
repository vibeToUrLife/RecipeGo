...
export function ShoppingListView({ items, roomId }: { items: ShoppingListRow[]; roomId?: string | null }) {
  ...
  function renameRow(id: string, name: string) {
    start(async () => {
      // Mirror the server's re-filing so the row moves to its new aisle at once
      // instead of hopping there when the revalidated list arrives.
      setOptimistic((prev) => prev.map((i) => (i.id === id
        ? { ...i, name, category: i.is_food ? categorizeIngredient(name) : i.category }
        : i)))
      try {
        const res = await updateItemNameAction(id, name)
        if (res?.error) toast.error(res.error)
      } catch {
        toast.error(t('shop.nameUpdateFailed'))
      }
    })
  }
  ...
  function renderRow(row: ShoppingListRow) {
    return (
      <li key={row.id} className="flex items-center gap-3 border-b py-2 text-sm">
        ...
        <NameEditor row={row} onCommit={renameRow} />
...
// Click-to-edit name for a row: the name reads as text until you click it, then
// becomes a field. Commits on blur or Enter; Escape reverts. Same validation the
// add-item form applies, so a rename can't produce a row the form couldn't.
function NameEditor({
  row,
  onCommit,
}: {
  ...
  // Re-sync the field when the stored name changes elsewhere (a revalidation, or
  // a rejected edit being reverted).
  const [synced, setSynced] = useState(row.name)
  // Escape has to beat the blur it triggers: the reverted value is still a
  // pending state update by the time commit() reads `val`, so flag it in a ref.
  ...
  function commit() {
    ...
    if (clean === row.name) {
      setVal(row.name) // normalise display (trims stray whitespace)
      return
    }
    onCommit(row.id, clean)
  }
  ...
  if (!editing) {
    return (
      <button
        ...
        title={t('shop.editName')}
        className={cn(
          'min-w-0 cursor-text truncate rounded px-1 py-0.5 text-left hover:bg-muted',
  ...
  return (
    <Input
      ...
      aria-label={t('shop.editName')}
      className="h-7 min-w-0 flex-1 px-2 py-0"
    />
  )
}
