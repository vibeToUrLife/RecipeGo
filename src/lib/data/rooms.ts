...
import type { Room, RoomInvite, RoomMember, MemberWithName, PendingInvite } from '@/lib/db-types'
...
export async function listMembers(roomId: string): Promise<MemberWithName[]> {
  ...
  return (members ?? []).map((m: RoomMember) => ({ room_id: m.room_id, user_id: m.user_id, role: m.role, joined_at: m.joined_at, display_name: names[m.user_id] ?? null }))
}
...
export async function listMyPendingInvites(): Promise<PendingInvite[]> {
  ...
  if (error) throw error
  return (data ?? []).map((r: PendingInvite) => ({
    id: r.id,
    room_id: r.room_id,
