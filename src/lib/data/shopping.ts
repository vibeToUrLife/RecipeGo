...
// Rename an item already on the list. A food row is re-filed into whichever
// aisle its new name belongs to, exactly as it would have been on the way in;
// "daily" rows are always Other, so their category is left alone. A rename that
// collides with another row stays a separate row rather than being folded in —
// the two may carry different units, and merging would swallow a quantity the
// user typed.
export async function setItemName(id: string, name: string): Promise<void> {
  const supabase = await createClient()
  ...
  const patch: { name: string; category?: Aisle } = { name: clean }
  if ((data as { is_food: boolean }).is_food) patch.category = categorizeIngredient(clean)
