...
[![CI](https://github.com/vibeToUrLife/RecipeGo/actions/workflows/ci.yml/badge.svg)](https://github.com/vibeToUrLife/RecipeGo/actions/workflows/ci.yml)

**[Live demo →](https://recipego-five.vercel.app)**

## What it is

RecipeGo is a multi-user recipe management app where every cook has their own private library. You can add recipes by hand or import them directly from any food blog URL — the app fetches the page, parses the ingredients and steps, and pre-fills the form so you only need to review and save. A built-in serving scaler adjusts every quantity when you change the number of portions. When you add recipes to your shopping list, RecipeGo merges shared ingredients across dishes (so two recipes each calling for garlic appear as one line with the combined amount) and groups everything by supermarket aisle so you can move through the store in order. A weekly **meal planner** lets you drop recipes into Breakfast / Lunch / Dinner slots for each day, set how many people each meal is for, and then build the whole week's shopping list in one tap — works for your personal recipes and inside shared Rooms. When it's time to actually cook, **cook mode** takes over the screen: one step at a time in large type, a tickable ingredient list scaled to your chosen servings, arrow-key and big-button navigation, and the screen held awake so it never dims mid-recipe. The interface follows a Warm Editorial design — clean typography, a cream-and-terracotta palette, and no visual clutter.

## Screenshots

<!-- Placeholder image paths — capture the screens listed in the commit notes and drop the
     PNGs into docs/screenshots/ using these exact filenames. Do not commit faked images. -->

|  |  |
|---|---|
| **Recipe library** <br> ![Recipe library](docs/screenshots/library-desktop.png) | **Recipe detail &amp; serving scaler** <br> ![Recipe detail](docs/screenshots/recipe-detail-desktop.png) |
| **Import from a URL** <br> ![Import from a URL](docs/screenshots/import-desktop.png) | **Weekly meal planner** <br> ![Weekly meal planner](docs/screenshots/meal-planner-desktop.png) |
| **Shopping list — merged &amp; aisle-grouped** <br> ![Shopping list](docs/screenshots/shopping-list-mobile.png) | **Live room sync** <br> ![Live room sync](docs/screenshots/room-sync-mobile.png) |
...
| DB migrations | Versioned SQL files in `supabase/migrations/` (profiles, recipes, storage, rooms, meal planner, realtime, and more) |
...
The database schema lives in versioned migration files under `supabase/migrations/`. Apply them to your hosted Supabase project:
...
`npx supabase db push` applies every migration in order (profiles, recipes, shopping list, storage, rooms, meal planner, realtime, and more). You only need to run this once per environment, or again when new migrations are added.
...
This runs 175 tests across 29 files covering business logic (serving scaler, fraction formatting, aisle grouping, ingredient merging, unit conversion), UI components (cook mode, the URL import bar, the week planner), and the URL import route handler. No network or database connection is required.
...
The project was security-audited; the original findings are in
[`AUDIT.md`](AUDIT.md). The main issue was an SSRF weakness in the URL-import
endpoint (`/api/import-recipe`): the target host was validated only as a string,
so a hostname that resolved to a private address (loopback, link-local, or the
cloud-metadata IP) — or a redirect to one — could make the server reach internal
services. It is fixed by resolving the host and rejecting
private/loopback/link-local/ULA addresses, pinning the socket to the validated IP
(closing the DNS-rebinding gap), and re-validating every redirect hop. The import
response is size-capped and the endpoint is rate-limited per user. Other audited
items (server-side image-upload limits, lint, CI) were addressed on the same
branch — see the commit history.
...
supabase/
  migrations/    Versioned SQL migration files applied with `npx supabase db push`
