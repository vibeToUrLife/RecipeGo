...
- **Source layout:** project scaffolded with `--src-dir` and import alias `@/*` → `./src/*`. All app code lives under `src/` (`src/app`, `src/components`, `src/lib`, `src/utils`). Middleware is `src/middleware.ts`. Supabase migrations live in `supabase/migrations/` at the repo root.
...
│  └─ middleware.ts                  # root middleware (T5)
├─ supabase/
